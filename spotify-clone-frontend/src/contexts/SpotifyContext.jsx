// spotify-clone-frontend/src/contexts/SpotifyContext.jsx
...
export const SpotifyProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (hash.access_token) {
      ...
      const expiry = Date.now() + parseInt(hash.expires_in) * 1000;
      setExpiresAt(expiry);
      localStorage.setItem('spotify_access_token', hash.access_token);
      localStorage.setItem('spotify_refresh_token', hash.refresh_token);
      localStorage.setItem('spotify_expires_at', expiry);
    } else {
  ...
  const refreshSpotifyToken = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('spotify_expires_at', newExpiry);
  ...
  // Updated playTrack: transfer then play
